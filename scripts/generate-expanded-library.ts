...
export async function generateExpandedLibrary(root: string): Promise<void> {
  ...
  await writeFile(path.join(root, "evals", "datasets", "routing-library-adversarial.yaml"), stringify({
    ...
    examples: [
      ...
      { id: "adv-summary-not-translation", prompt: "Summarize this document for executives while preserving caveats; do not translate it.", expectedCategory: "documents-communication", expectedAtoms: ["atom-summarize-document"], mustNotRoute: ["atom-translate-document"] },
      { id: "adv-clean-not-analyze", prompt: "Clean this dataset by normalizing columns and deduplicating rows; do not interpret the business metrics.", expectedCategory: "data-analytics", expectedAtoms: ["atom-clean-tabular-data"], mustNotRoute: ["atom-analyze-tabular-data"] },
      { id: "adv-schema-not-migration", prompt: "Design tables, relationships, keys, and constraints for a new database; do not migrate live data.", expectedCategory: "software-design", expectedAtoms: ["atom-design-database-schema"], mustNotRoute: ["atom-plan-database-migration"] },
      { id: "adv-roadmap-not-requirements", prompt: "Prioritize these feature opportunities by outcome, evidence, effort, and risk; do not write detailed requirements.", expectedCategory: "product-definition", expectedAtoms: ["atom-prioritize-product-roadmap"], mustNotRoute: ["atom-write-product-requirements"] },
      { id: "adv-copy-not-strategy", prompt: "Write landing page copy for this defined audience and offer; do not design the whole content strategy.", expectedCategory: "business-growth", expectedAtoms: ["atom-write-marketing-copy"], mustNotRoute: ["atom-plan-content-strategy"] },
      { id: "adv-threat-not-audit", prompt: "Create a threat model with assets, trust boundaries, actors, and abuse paths; do not scan source code.", expectedCategory: "security-trust", expectedAtoms: ["atom-create-threat-model"], mustNotRoute: ["atom-audit-source-security"] },
      { id: "adv-runbook-not-live", prompt: "Write an operational runbook with diagnostics, stop conditions, rollback, and escalation; do not execute production commands.", expectedCategory: "automation-operations", expectedAtoms: ["atom-write-operational-runbook"], mustNotRoute: ["atom-triage-production-incident"] },
      { id: "adv-diagram-not-architecture", prompt: "Draw the approved architecture as an editable technical diagram; do not invent system boundaries.", expectedCategory: "design-media", expectedAtoms: ["atom-create-technical-diagram"], mustNotRoute: ["atom-design-software-architecture"] },
      { id: "adv-image-edit-not-generate", prompt: "Edit this supplied image by removing the background while preserving the subject; do not generate a new scene.", expectedCategory: "design-media", expectedAtoms: ["atom-edit-image"], mustNotRoute: ["atom-generate-image"] },
      { id: "adv-notes-not-vault", prompt: "Turn this meeting transcript into structured notes with decisions and actions; do not reorganize the whole knowledge vault.", expectedCategory: "personal-productivity", expectedAtoms: ["atom-capture-structured-notes"], mustNotRoute: ["atom-operate-knowledge-vault"] },
      { id: "adv-peer-review-not-rewrite", prompt: "Peer review this manuscript for methods, claims, and reproducibility; do not rewrite it.", expectedCategory: "scientific-research", expectedAtoms: ["atom-peer-review-manuscript"], mustNotRoute: ["atom-edit-prose"] }
    ]
  }), "utf8");
  await writeFile(path.join(root, "evals", "datasets", "task-completion-library.yaml"), stringify({
    ...
    examples: [
      ...
      { id: "task-data-cleaning", prompt: "Clean this customer table reproducibly.", expectedCategory: "data-analytics", expectedAtoms: ["atom-clean-tabular-data"], mustNotRoute: ["atom-analyze-tabular-data"], task: "Profile quality, define normalization rules, preserve raw inputs, apply reproducible transformations, and validate keys, types, row counts, and anomalies.", rubric: ["Profiles data quality", "Declares transformation rules", "Preserves source lineage", "Validates the cleaned result"] },
      { id: "task-threat-model", prompt: "Create a threat model for this service.", expectedCategory: "security-trust", expectedAtoms: ["atom-create-threat-model"], mustNotRoute: ["atom-audit-source-security"], task: "Map assets, actors, flows, entry points, trust boundaries, abuse cases, controls, residual risk, and prioritized verification.", rubric: ["Defines assets and objectives", "Maps trust boundaries", "Prioritizes realistic abuse paths", "Links mitigations to verification"] },
      { id: "task-launch-plan", prompt: "Plan this product launch.", expectedCategory: "business-growth", expectedAtoms: ["atom-plan-product-launch"], mustNotRoute: ["atom-plan-software-release"], task: "Create a launch plan with audience, positioning, channels, readiness gates, owners, metrics, risks, and fallback actions.", rubric: ["Defines audience and positioning", "Assigns readiness ownership", "Specifies measurable outcomes", "Includes risks and fallback actions"] },
      { id: "task-observability", prompt: "Design observability for this service.", expectedCategory: "automation-operations", expectedAtoms: ["atom-design-observability"], mustNotRoute: ["atom-triage-production-incident"], task: "Define user journeys, SLIs, SLOs, telemetry, dashboards, actionable alerts, ownership, retention, cost, and review cadence.", rubric: ["Connects signals to user outcomes", "Defines SLOs and telemetry", "Makes alerts actionable", "Addresses ownership and cost"] },
      { id: "task-learning-path", prompt: "Plan a measurable learning path for this goal.", expectedCategory: "personal-productivity", expectedAtoms: ["atom-plan-learning-path"], mustNotRoute: ["atom-build-action-plan"], task: "Assess baseline, decompose prerequisites, sequence resources and practice, and define feedback, spaced review, checkpoints, and adaptation rules.", rubric: ["Uses learner baseline and constraints", "Sequences prerequisites and practice", "Includes feedback and retrieval", "Defines adaptive checkpoints"] },
      { id: "task-peer-review", prompt: "Peer review this scientific manuscript.", expectedCategory: "scientific-research", expectedAtoms: ["atom-peer-review-manuscript"], mustNotRoute: ["atom-edit-prose"], task: "Summarize claims and methods, identify evidence-located major and minor issues, assess reproducibility and ethics, and ask constructive questions without an editorial verdict.", rubric: ["Accurately frames claims and methods", "Locates and prioritizes findings", "Checks reproducibility and ethics", "Avoids rewriting or editorial verdicts"] }
    ]
  }), "utf8");
}
